...
7. **M6 — Polish + AppImage release**: real icon, MIME registration, recent files. Also: **non-English environment verification** — exercise the app under `LANG=es_ES.UTF-8` / `LANG=ru_RU.UTF-8` (and other locales) to catch number/date parsing assumptions, font-mapper hits for Cyrillic / accented Latin glyphs, IME composition behavior in the AcroForm `FormLayer`, and any `lp` / `lpstat` output we parse (`request id is …`, lpoptions key/value lines) that gets localized by CUPS in some distros. Decide per-finding whether to fix or to document as a known limitation.
