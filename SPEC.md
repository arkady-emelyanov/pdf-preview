...
- **Text selection** ✅: a `Text` tool (`T`) flips each page's overlay into a text-cursor mode. Drag selects char range; **Edit → Copy** (`Ctrl+C`) and right-click → Copy both copy the slice; `Esc` clears. Driven by a new `pdf:getChars` IPC that returns `{text, boxes}` from `FPDFText_GetCharBox` (per-char box in PDF page coords). The renderer caches per (sourceId, sourceIndex), so revisiting a page is instant. Main tracks per-window has-selection state (via `pdf:setHasTextSelection`) to grey out the context menu's Copy item when nothing's selected; input fields fall through to the platform's native copy. v1 limit: single-page selection only.
...
6. **M5 — Print** ✅: CUPS pipeline (`lpstat -a`/`-p` enumeration, `lpoptions -l` capability discovery, bake-via-pdf-lib → `lp -d … -o …`), custom modal in the renderer with printer / copies / page-range / odd-even / capability-driven duplex+paper+color / orientation / scaling, paper-sized live preview that suppresses the form-field tint, job-status footer with cancel. Menu: File → Print… (`Ctrl+P`).
7. **M6 — Polish + AppImage release** ✅: recent files; window position/size restore (saved to `window.json` in the user config dir, off-screen clamped via `screen.getDisplayMatching`); MIME registration (first-run, packaged-only: writes a `.desktop` to `~/.local/share/applications` referencing the running AppImage, runs `update-desktop-database` + `xdg-mime default … application/pdf`, idempotent via a flag file); annotation author stamped on creation from `git config --global user.name` falling back to `$USER`, surfaced through `DocInfo.author` and applied in the store's `addAnnotation` only when the annotation doesn't already carry one (so paste preserves the original); Export Flattened Copy (forces the pdf-lib path with `form.flatten()`) and Export Pages as Images (PDFium render → RGBA→BGRA swap → `nativeImage.toPNG()`); CUPS tools (`lpstat`/`lp`/`lpoptions`/`cancel`) are spawned with `LC_ALL=C LANG=C` so a non-English locale doesn't break our regex-based output parsing (the `accepting requests` / `printer X is` / `system default destination:` / `request id is` strings are all CUPS-translatable). Config storage lives under `~/.config/pdf-preview` (XDG-aware), set via `app.setPath('userData', …)` before any module reads it. Known limitations (not fixed in v1): real app icon is still the placeholder `resources/icon.png`; IME composition inside the AcroForm `FormLayer` has not been exercised end-to-end — should be characterized before release.
